 ...
 */
class ContainsDefinitionSparql {
    /**
...
EOF;
    ...
    protected function generateConceptSearchQueryCondition($term, $searchLang)
    {
        ...
        if (preg_match('/^[^\*]+$/', $term)) { // exact query
            ...
            $term = str_replace('\\', '\\\\', $term); // quote slashes
            $term = str_replace('\'', '\\\'', mb_strtolower($term, 'UTF-8')); // make lowercase and escape single quotes
            $filtercond = "CONTAINS(LCASE(STR(?match)), '$term')";
   ...
   WHERE {
    $graphClause {
     { 
     $valuesProp
     VALUES (?prop ?pri ?langParam) { (skos:prefLabel 1 $langClause) (skos:altLabel 3 $langClause) (skos:notation 5 '') (skos:hiddenLabel 7 $langClause) (skos:definition 9 $langClause)}
     $textcond
     ?s ?prop ?match }
     OPTIONAL {
